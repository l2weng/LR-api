...
router.post('/create', (req, res) => {
  const WindVaneCreation = async done => {
    const WindVaneAddResult = await Feedback.create({
...
router.post('/update', (req, res) => {
  const WindVaneUpdate = async done => {
    const windVaneUpdateResult = await Feedback.update(
      {
        name: req.body.name && req.body.name.trim(),
      },
      {
...
router.get('/downloadConfig', (req, res) => {
  const vaneId = req.query.vaneId;
  const type = req.query.type;
  const routingInspectId = req.query.routingInspectId;
  console.log(req.query);

  const dJson = {};
  Feedback.findById(vaneId).then(windVane => {
    Task.findAll({
      where: {
        windVaneId: parseInt(vaneId),
        type: parseInt(type),
        routingInspectId: parseInt(routingInspectId),
      },
      order: [['sort', 'ASC']],
    }).then(fanPhotos => {
      ...
      dJson.businessReferer =
        'http://windapi-server.chinanorth.cloudapp.chinacloudapi.cn/wind/';
